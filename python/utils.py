"""Utils for Echo display - simplified from whisplay."""
import os
import unicodedata
import numpy as np
...
class EmojiUtils:
    ...
    def get_local_emoji_svg_image(char, size):
        if not HAS_CAIROSVG:
            return None
        filename = "-".join(f"{ord(c):x}" for c in char) + ".svg"
        path = os.path.join(os.path.dirname(__file__), "emoji_svg", filename)
    ...
    def get_line_img(text, font):
        ...
        for char in text:
            if EmojiUtils.is_emoji(char):
                emoji_img = EmojiUtils.get_local_emoji_svg_image(char, size=font.size)
                if emoji_img:
                    emoji_y = ascent - emoji_img.height
